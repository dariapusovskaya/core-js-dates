...
function getWeekNumberByDate(date) {
  ...
  const daysToThursday = 3 - isoDayOfWeek;
  ...
  const firstThursday = new Date(
    ...
    0,
    1 + daysToFirstThursday
  );
  if (thursday < firstThursday) {
    const lastDayOfPrevYear = new Date(thursday.getFullYear() - 1, 11, 31);
    return getWeekNumberByDate(lastDayOfPrevYear);
  }
  const diffInMs = thursday - firstThursday;
  const msInWeek = 7 * 24 * 60 * 60 * 1000;
  const weekNumber = Math.floor(diffInMs / msInWeek) + 1;
  return weekNumber;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  for (
    ...
    if (positionInCycle < countWorkDays) {
      result.push(formatDateString(new Date(currentDate)));
    }
  }
  return result;
}
